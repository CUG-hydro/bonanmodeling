...
for i = 1:nsoi
  ...
  tsoi0 = Tsoil(i);           % Soil temperature before phase change
  ...
  if (soilvar.h2osoi_ice(i) > 0 & Tsoil(i) > physcon.tfrz)
    imelt = 1;
    Tsoil(i) = physcon.tfrz;
  end
  
  % Freezing: if liquid exists below melt point, freeze some to ice.
  % Identify freezing by imelt = 2
  if (soilvar.h2osoi_liq(i) > 0 && Tsoil(i) < physcon.tfrz)
    imelt = 2;
    Tsoil(i) = physcon.tfrz;
  end
  ...
  if (imelt > 0)
    heat_flux_pot = (Tsoil(i) - tsoi0) * soilvar.cv(i) * soilvar.dz(i) / dt;
  else
    heat_flux_pot = 0;
  end
  ...
  if (imelt > 0)
    ...
    residual = heat_flux_pot - heat_flux; % update
