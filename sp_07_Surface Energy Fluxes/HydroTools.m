classdef HydroTools
  ...
  methods (Static)
    ...
    function [fluxvar, fx] = most (physcon, forcvar, surfvar, fluxvar, x)
      ...
      [psi_m_zref] = HydroTools.psi_m_monin_obukhov (z_minus_d / x);
      [psi_m_z0m] = HydroTools.psi_m_monin_obukhov (fluxvar.z0m / x);
      psim = -psi_m_zref + psi_m_z0m;
      
      % --- Evaluate psi for scalars at the reference height (zref-disp) and surface (z0c)
      [psi_c_zref] = HydroTools.psi_c_monin_obukhov (z_minus_d / x);
      [psi_c_z0c] = HydroTools.psi_c_monin_obukhov (fluxvar.z0c / x);
