...
fluxvar.gam = forcvar.rhomol * fluxvar.ustar * fluxvar.ustar / forcvar.uref; % rho * ustar^2 / uref
fluxvar.gac = forcvar.rhomol * fluxvar.ustar * fluxvar.tstar / (forcvar.thref - fluxvar.tsrf); % rho * ustar * tstar / (thref - tsrf)
...
dlwrad = 4 * surfvar.emiss * physcon.sigma * fluxvar.tsrf^3; % 4 * sigma * Ts^3, Eq. 7.16

% Sensible heat flux (W/m2) and temperature derivative (W/m2/K)
fluxvar.shflx = forcvar.cpair * (fluxvar.tsrf - forcvar.thref) * fluxvar.gac; % Cp * (Ts - Ta) * gac
...
f0 = fluxvar.qa - fluxvar.lwrad - fluxvar.shflx - fluxvar.lhflx; % Net energy flux into soil, G
df0 = -dlwrad - dshflx - dlhflx; % 其余的能量
...
dtsrf = soilvar.tsoi(1) - tsoi0(1); % Change in surface temperature (K), 这里是对温度的偏导
